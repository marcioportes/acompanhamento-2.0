...
describe('#101 — trade já enriquecido ainda ganha o apontador pós-saída', () => {
  it('com MEP/MEN e SEM postExit, calcula só o apontador em vez de pular', async () => {
    // 20 trades da base já tinham excursão. O early return os deixaria sem o
    // apontador para sempre.
    const update = vi.fn(async () => {});
    const trade = {
      studentId: 'a1', ticker: 'MNQZ26', side: 'LONG', entry: 100, stopLoss: 90,
      entryTime: '2026-08-24T10:00:00', exitTime: '2026-08-24T10:30:00',
      planId: 'p1', mepPrice: 120, menPrice: 95, excursionSource: 'yahoo',
    };
    const db = {
      collection: (nome) => ({
        doc: () => ({
          get: async () => ({
            exists: true,
            data: () => (nome === 'plans' ? { rrTarget: 2 } : trade),
          }),
          update,
        }),
      }),
    ...
    const fetchFn = async () => ({
      ok: true,
      json: async () => ({
        chart: { result: [{ timestamp: [1787580000], indicators: { quote: [{ high: [125], low: [118] }] } }] },
      }),
    });

    const r = await runEnrichment({ tradeId: 't1' }, { db, fetchFn });
    expect(r.skipped).toBe(true);
    expect(r.postExit?.outcome).toBe('ALVO'); // alvo = 100 + 10*2 = 120; a barra tocou 125
    expect(update).toHaveBeenCalledTimes(1);
    expect(update.mock.calls[0][0]).toHaveProperty('postExit');
  });

  it('com MEP/MEN E postExit já gravados, é no-op de verdade', async () => {
    const update = vi.fn(async () => {});
    const trade = {
      mepPrice: 120, menPrice: 95, excursionSource: 'yahoo',
      postExit: { outcome: 'STOP', source: 'yahoo' },
    };
    const db = { collection: () => ({ doc: () => ({ get: async () => ({ exists: true, data: () => trade }), update }) }) };
    const r = await runEnrichment({ tradeId: 't1' }, { db });
    expect(r.skipped).toBe(true);
