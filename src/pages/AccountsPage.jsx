...
import { 
  Plus, Wallet, Edit2, Trash2, ShieldCheck, FlaskConical, Trophy, X, Search, Building2, FileText, ChevronRight
} from 'lucide-react';
...
const AccountsPage = () => {
  ...
  const { brokers, loading: brokersLoading } = useMasterData();
  
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingAccount, setEditingAccount] = useState(null);
  const [showBrokerSuggestions, setShowBrokerSuggestions] = useState(false);
  const [selectedAccount, setSelectedAccount] = useState(null); // Para página de detalhes
  ...
  // Filtro de sugestões de corretora (do Firestore)
  ...
  const filteredBrokers = useMemo(() => {
    ...
    return brokerNames.filter(b => 
      b.toLowerCase().includes(formData.broker.toLowerCase())
    ).slice(0, 8);
  ...
  const openModal = (account = null) => {
    if (account) {
      ...
      setFormData({
        name: account.name || '',
        broker: account.broker || account.brokerName || '', // Fallback para brokerName
  ...
  const handleSubmit = async (e) => {
    ...
    // Calcular isReal para escrita híbrida
    const isRealDerived = formData.type === 'REAL' || formData.type === 'PROP';
    
    const payload = {
      name: formData.name.trim(),
      broker: formData.broker.trim(),      // Salvar como 'broker' (nome)
      brokerName: formData.broker.trim(),  // Também salvar como brokerName para compatibilidade
      currency: formData.currency,
      initialBalance: Number(formData.initialBalance),
      type: formData.type,
      isReal: isRealDerived,               // Escrita híbrida
    };

    // Se não está editando, incluir currentBalance inicial
    if (!editingAccount) {
  ...
  const getTypeLabel = (type) => {
    switch(type) {
      case 'REAL': return 'Conta Real';
      case 'PROP': return 'Mesa Proprietária';
      default: return 'Demo/Simulado';
    }
  };
  ...
  // Se tem conta selecionada, mostrar página de detalhes
  if (selectedAccount) {
    return (
      <AccountDetailPage 
          ...
          >
            ...
            <div className="space-y-3 border-t border-slate-800 pt-4">
              ...
              <div className="flex justify-between items-center">
                ...
                {(() => {
                  // Usar currentBalance se existir, senão initialBalance
                  const saldoAtual = acc.currentBalance !== undefined ? acc.currentBalance : (acc.initialBalance || 0);
            ...
            <div 
              ...
              onClick={(e) => e.stopPropagation()} // Evita navegação ao clicar nos botões
            >
              <button 
