...
// ============================================
// MATCHED OPERATIONS DETECTION
// ============================================

/**
 * Identifica operações reconstruídas que TÊM trade correspondente
 * (opostas das ghost operations).
 *
 * @param {Object[]} operations — reconstructedOps
 * @param {Object[]} correlations — output de correlateOrders().correlations
 * @param {Object[]} trades — trades existentes do plano
 * @returns {Array<{ operation, trade, correlations }>}
 */
export function identifyMatchedOperations(operations, correlations, trades) {
  if (!operations?.length || !correlations?.length || !trades?.length) return [];

  // Indexar trades por ID para lookup O(1)
  const tradesById = new Map(trades.map(t => [t.id, t]));

  // Correlações não-ghost (matchType === 'exact' ou 'best')
  const matchedCorrelations = correlations.filter(c => c.matchType !== 'ghost' && c.tradeId);

  if (matchedCorrelations.length === 0) return [];

  const result = [];

  for (const op of operations) {
    if (op._isOpen) continue; // Operações abertas não podem ser comparadas
    ...
    // Coletar todos os tradeIds únicos das correlações das ordens desta operação
    const tradeIdsForOp = new Set();
    for (const order of filledOrders) {
      const orderInstrument = (order.instrument || '').toUpperCase();

      for (const corr of matchedCorrelations) {
        const corrInstrument = (corr.instrument || '').toUpperCase();
        if (corrInstrument !== orderInstrument) continue;

        if (corr.externalOrderId && order.externalOrderId) {
          if (corr.externalOrderId === order.externalOrderId) {
            tradeIdsForOp.add(corr.tradeId);
          }
        } else if (corrInstrument === orderInstrument) {
          // Fallback: associa por instrumento (menos preciso, mas cobre parsers sem externalOrderId)
          tradeIdsForOp.add(corr.tradeId);
        }
      }
    }

    // Se a operação matcha com exatamente 1 trade, é um candidato claro para confronto
    // Se matcha com múltiplos ou zero, pular (ambíguo ou ghost)
    if (tradeIdsForOp.size === 1) {
      const tradeId = [...tradeIdsForOp][0];
      const trade = tradesById.get(tradeId);
      if (trade) {
        result.push({ operation: op, trade });
      }
    }
  }
...
/**
 * Prepara batch de operações correlacionadas para confronto: identifica,
 * compara e filtra apenas as que têm divergências para apresentar ao aluno.
 *
 * @param {Object[]} operations — reconstructedOps
 * @param {Object[]} correlations — output de correlateOrders().correlations
 * @param {Object[]} trades — trades existentes do plano
 * @returns {{ divergent: Array, converged: Array }}
 *   divergent: [{ operation, trade, comparison }] — operações com divergências
 *   converged: [{ operation, trade }] — operações sem divergências (já alinhadas)
 */
export function prepareConfrontBatch(operations, correlations, trades) {
  const matched = identifyMatchedOperations(operations, correlations, trades);

  const divergent = [];
  const converged = [];

  for (const { operation, trade } of matched) {
