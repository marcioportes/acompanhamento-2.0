/**
 * orderProtection.js — issue #466 (épico #462 F3)
 *
 * SSoT de "perna" e "proteção", lado cliente. Espelho ESM de
 * `functions/shared/orderProtection.js` — o CORPO é idêntico, só muda o import e o export
 * (paridade testada em `src/__tests__/functions/shared/orderProtectionMirror.test.js`).
 *
 * POR QUE EXISTE: até o #466 havia cinco definições de "proteção/stop" — `isStopOrder`
 * (parser, pelo tipo), `stopSemantic` (contra o limite da 1ª entrada),
 * `ehProtecaoAdversa` (contra o médio, com atalho por `isStopOrder`), `protectiveLegsOf`
 * (motor e espelho, sem "nasceu com a posição") e o "último da lista" de
 * `orderTradeCreation`. Cada uma dava um stop diferente para o mesmo trade: o 24/09/2026
 * saiu com risco de R$ 7.070 porque o stop veio de uma ordem cancelada 44 min ANTES da
 * entrada. Quem precisa saber se uma ordem protege uma posição passa por aqui.
 *
 * Instantes: sempre pelo helper do #464 (`instantAtOffsetMs`), nunca `new Date(naive)`.
 */
