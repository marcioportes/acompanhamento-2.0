 ...
 */
const { deveBloquear } = require('../shared/loginBlockPolicy');
...
const buildReportHtml = (today, sections, summaryData) => {
  ...
  // #101 — seção própria: rebloqueio não é "novo inadimplente", é uma decisão sua
  // sendo revertida. Precisa aparecer nomeada, não diluída na lista de devedores.
  const renderReblocked = (items = []) => {
    if (!items.length) return '';
    const rows = items
      .map((s) => `<p style="color:#e2e8f0;font-size:14px;margin:4px 0;">&bull; <strong>${s._studentName}</strong> — login bloqueado de novo (segue inadimplente)</p>`)
      .join('');
    return `<tr><td style="padding:24px 32px 0;"><h3 style="color:#f97316;font-size:14px;text-transform:uppercase;letter-spacing:1px;margin:0 0 8px;border-bottom:1px solid #334155;padding-bottom:8px;">Rebloqueados</h3>${rows}</td></tr>`;
  };
...
${renderReblocked(sections.reblocked)}
  ...
  .onRun(async () => {
    ...
    try {
      ...
      const sections = {
        ...
        newOverdue: [],
        existingOverdue: [],
        // #101 — quem a rotina rebloqueou nesta execução. O mentor precisa saber
        // que um desbloqueio dele foi revertido, e por quê.
        reblocked: [],
      };
      ...
      for (const studentDoc of studentsSnap.docs) {
        ...
        let bestTier = 'none';
        // #101 — o autobloqueio virou INVARIANTE, não evento: enquanto houver
        // assinatura vencida e nenhuma viva, o login fica bloqueado. Antes, o
        // bloqueio só acontecia no dia da transição active→overdue; desbloquear
        // uma vez dava acesso permanente a quem seguia devendo.
        ...
        for (const subDoc of subsSnap.docs) {
          ...
          if (sub.type === 'trial' && sub.status === 'active') {
            ...
            // Trial que ainda não expirou é acesso vivo; expirado cai no update abaixo.
            if (!trialEnd || daysBetweenSigned(today, trialEnd) >= 0) temViva = true;
            if (trialEnd) {
          ...
          if (sub.type !== 'trial' && sub.status === 'active') {
            ...
            if (renewalDate) {
              ...
              // Assinatura paga ativa sem data de renovação: não há vencimento a cobrar.
              temViva = true;
            }
        ...
        // #101 — invariante do bloqueio, verificado todo dia. O bloqueio MANUAL do
        // mentor não é tocado aqui (ele só some se o próprio mentor desfizer); o que
        // esta regra faz é impedir que um desbloqueio deixe inadimplente com acesso
        // para sempre.
        if (deveBloquear({ temVencida, temViva, bloqueado: student.loginBlocked === true })) {
          batch.update(studentDoc.ref, {
            loginBlocked: true,
            loginBlockedAt: admin.firestore.FieldValue.serverTimestamp(),
            loginBlockedBy: 'system:checkSubscriptions',
          ...
          batchCount++;
          if (!authDisableQueue.includes(studentDoc.id)) authDisableQueue.push(studentDoc.id);
          sections.reblocked.push({
