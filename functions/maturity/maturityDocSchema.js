...
// ============================================
//
// ⚠️ ESPELHO de src/utils/maturityEngine/maturityDocSchema.js — MANTER SINCRONIZADO ⚠️
...
function validateCurrentDoc(doc) {
  const errors = [];
...
function validateHistoryDoc(doc) {
  const errors = [];
  if (!isPlainObject(doc)) {
    return { valid: false, errors: [`doc must be object (got: ${fmt(doc)})`] };
  }
...
module.exports = { validateCurrentDoc, validateHistoryDoc };
