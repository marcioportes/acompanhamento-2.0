...
> **Versão:** 0.29.0  
> **Última atualização:** 22/04/2026 — v0.29.0: Abertura #169 scripts Protocolo Autônomo §13 Fases A+B+C — `cc-notify-email.py` (iCloud SMTP + rate limit 4h), `cc-validate-task.py` (3 checks INV-27), refactor `cc-worktree-start.sh` 3-tier com `.coord-dir` + pré-condição cwd=worktree + template `worker-briefing.md`, `.env.example` em `~/cc-mailbox/`. Fase D (rodada end-to-end) separada. Sem chunk lock (infra fora do produto). Sem bump em `src/version.js` (scripts não são produto). Branch `arch/issue-169-scripts-protocolo-autonomo`.  
