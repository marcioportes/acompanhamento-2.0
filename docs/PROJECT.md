...
> **Versão:** 0.32.0  
> **Última atualização:** 23/04/2026 — v0.32.0: Abertura #174 wire setupsMeta em MentorDashboard (fix E4 out-of-scope de #170) — lock CHUNK-16 (escrita). v1.42.1 reservada em version.js como patch. Branch `fix/issue-174-mentor-setups-meta`. Escopo mínimo: importa `useSetups` no MentorDashboard + filtra por `selectedStudent.uid` + passa `setupsMeta={studentSetups}` ao `<SetupAnalysis>`. ~15 linhas + teste mínimo. Origem: detectado por Marcio no review pós-merge do #170; eu havia cortado o wire sem discutir, violação explícita registrada em memória operacional.  
> **Última atualização (histórica):** 23/04/2026 — v0.31.0: Encerramento #170 v1.42.0 SetupAnalysis V2 (PR #173 mergeado, merge commit `15a6dca3`). Substitui `SetupAnalysis.jsx` por card de diagnóstico operacional com 4 KPIs em grid 2×2 (EV por trade · Payoff · ΔT W vs L com semáforo ±20%/±10% · Contribuição ao EV total). Aderência RR condicional (só quando `setups.targetRR`), Sparkline 6m, Insight 1-linha, ordenação por \|contribEV\| desc, accordion "Esporádicos (N)" para setups com n<3. Util puro `src/utils/setupAnalysisV2.js`. Zero campo Firestore novo. Consumido em `StudentDashboard` via `useSetups` já presente. MentorDashboard sem `setupsMeta` (não filtra por student, fast-follow). Fast-fix `0bffe1f1` pré-merge: overflow do card corrigido (header em 2 linhas, truncate no nome, sublabels curtos, `Xm` em vez de `Xmin`). 1880/1880 testes (+40: 23 util + 17 render). Lock CHUNK-02 liberado (AVAILABLE). Issue doc arquivada. Worktree `~/projects/issue-170` removido.  
...
|-------|-------|--------|------|--------|
| CHUNK-16 | #174 | `fix/issue-174-mentor-setups-meta` | 23/04/2026 | Wire setupsMeta em MentorDashboard (fix E4 out-of-scope #170) |
