...
> **Versão:** 0.34.0  
> **Última atualização:** 23/04/2026 — v0.34.0: Abertura #176 scripts de orquestração §13 (coord-briefing + cc-spawn-coord + cc-dispatch-task) — meta-infra do Protocolo Autônomo, fecha os gaps do §13.8 passos 8b/8d identificados no review pós-#169. Sem chunk lock (infra fora do produto). Sem bump em `src/version.js`. Branch `arch/issue-176-scripts-orquestracao-protocolo`.  
