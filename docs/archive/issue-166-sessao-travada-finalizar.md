...
> **Versão entregue:** v1.40.0
> **PR:** #168 (merge commit `ca74b289`) — mergeado 21/04/2026
> **Status:** ENCERRADO
> **Milestone:** v1.1.0 — Espelho Self-Service
> **Prioridade:** Sev1
> **Chunks:** CHUNK-09 (escrita) — liberado
...
- [x] Botão "Finalizar" responde ao clique com loading state visível
- [x] Cliques múltiplos durante execução são ignorados (disabled enquanto processa)
- [x] Erro em `completeAllProbing()` é capturado e exibe mensagem ao usuário (não trava silenciosamente)
- [x] Fluxo completo testado: clique → loading → success → `onComplete()` chamado
- [x] Teste cobrindo o comportamento do botão (loading, disabled, error)
...
|---|------|--------|--------|--------|
| 01 | Discovery — mapear fluxo completo do botão + causa raiz + propor fix | ✅ concluído | — | `01-discovery-report.md` |
| 02 | Implementação — fix loading state + disabled + error handling + testes | ✅ concluído | `4581eec6` / `235d1000` | `02-implementacao-report.md` |
