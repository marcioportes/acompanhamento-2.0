...
- R2.B18 [wizard-render-loop] commit `59024f37` ok — Smoke do banner B17 expôs `Maximum update depth exceeded` no wizard. Causa: 8 callbacks (onSnapshot/onMetrics/onPatterns/onAar/onSwot/onMaturity/onForward/onNotes) eram arrow inline `(x) => replaceSection('snapshot', x)` recriadas a cada render; step interno tratava como dep de useEffect → refire → setState → re-render → nova arrow → loop. Fix: `useCallback([replaceSection])` em todos os 8 (replaceSection já estável em `useCycleClosureDraft.js:186`). DEC-AUTO-259-16.
